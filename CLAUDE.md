...
- A post must exist in BOTH locales with the same filename; slugs are read from the `es` directory (`src/lib/blog.ts`)
- Content parsed by `parseMarkdown()` in the blog post page
- Routes: `/blog` (list), `/blog/[slug]` (post detail)
- Featured post is always the most recent by date (`getFeaturedPost` returns `posts[0]`); the blog grid excludes it by slug
